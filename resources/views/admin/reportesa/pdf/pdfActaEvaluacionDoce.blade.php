...
<body>
    <br>
    <span style="position: absolute; top: 10px; right: 10px; font-weight: bold; font-size: 11px;"> {{ $fechaHora->format('d-m-Y H:i:s') }} </span>
    ...
    <table>
        ...
        <tr id="color">
            ...
            <td style="width:15%" id="color" class="text-center" rowspan="2">{{ $fechaHora->format('d-m-Y') }}</td>
        </tr>
    ...
    <table style="width:100%; border-collapse: collapse;">
        <thead>
            <tr id="color">
                <th id="color" style="width: 4%;">N° Orden</th>
                <th id="color" style="width: 10%;">N° Matrícula</th>
                <th id="color" style="width: 48%;">APELLIDOS Y NOMBRES (Por Orden Alfabético)</th>

                @for ($i = 0; $i < $num_Competen->num_competencias; $i++)
                    <th id="color" style="width: 7%;">Calificación {{ $i + 1 }}.</th>
                @endfor

                {{-- <th id="color" style="width: 7%;">Calificación 1.</th>
                <th id="color" style="width: 7%;">Calificación 2.</th>
                <th id="color" style="width: 7%;">Calificación 3.</th> --}}
            ...
            @foreach ($listAlumnos as $listAlumno)
                ...
                <tr id="color">
                    ...
                    @for ($i = 0; $i < $num_Competen->num_competencias; $i++)
                        @switch($i + 1)
                            @case(1)
                                <td style="width: 15%; font-size:9px" id="color" class="text-center">{{ $listAlumno->idestado_matricula == 2 ? 'LICENCIA' : ($listAlumno->cal1 == 'ninguno' ? '-' : $listAlumno->cal1) }}</td>
                                @break

                            @case(2)
                                <td style="width: 15%; font-size:9px" id="color" class="text-center">{{ $listAlumno->idestado_matricula == 2 ? 'LICENCIA' : ($listAlumno->cal2 == 'ninguno' ? '-' : $listAlumno->cal2) }}</td>
                                @break

                            @case(3)
                                <td style="width: 15%; font-size:9px" id="color" class="text-center">{{ $listAlumno->idestado_matricula == 2 ? 'LICENCIA' : ($listAlumno->cal3 == 'ninguno' ? '-' : $listAlumno->cal3) }}</td>
                                @break
