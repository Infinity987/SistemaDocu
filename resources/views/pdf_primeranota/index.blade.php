...
@section('content')
    ...
    <div class="callout callout-danger mb-0">
        <section class="content-header p-0">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h1><i class="fas fa-table"></i> TABLA DE RESULTADOS PRIMERA NOTA</h1>
                    </div>
                    <div class="col-sm-6">
    ...
    <section class="ftco-section">
        <div class="container mt-4">
            <div class="row">
                <div class="col-md-12">
                    <h4 class="text-center mb-4">LISTA POR PROCESOS</h4>
                                ...
                                <tbody>
                                    @foreach ($procesosresultadoprimera as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <form action="{{ route('pdf.fichaprimeranota') }}" method="POST"
                                                    enctype="multipart/form-data">
                                                    @csrf
                                                    <button class="btn btn-info" data-toggle="tooltip" data-placement="top"
                                                        title="Generar PDF" type="submit" id="idprimeranota"
                                                        name="idprimeranota" value="{{ $item->idnota1 }}"><i
                                                            class="fas fa-file-pdf"></i>&nbsp;Generar ficha</button>
                                                </form>
                                            </td>
                                            <td>
                                                <form action="{{ route('excel.resultadoprimera') }}" method="POST">
                                                    @csrf
                                                    <button class="btn btn-success" data-toggle="tooltip"
                                                        data-placement="top" title="Exportar a Excel" type="submit"
                                                        name="idnota1" value="{{ $item->idnota1 }}">
                                                        <i class="fas fa-file-excel"></i>&nbsp;Exportar Excel
                                                    </button>
                                                </form>
                                            </td>
                                            <td>
                                                <form action="{{ route('resultadoprimera.eliminar') }}" method="POST"
                                                    class="form-eliminar">
                                                    @csrf
                                                    <input type="hidden" name="idnota1" value="{{ $item->idnota1 }}">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        title: '¿Estás seguro?',
                        text: "Una vez eliminada, no podrás recuperar esta nota.",
                        icon: 'warning',
