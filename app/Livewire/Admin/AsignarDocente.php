...
class AsignarDocente extends Component
{
    ...
    public function traerCursos($selectTipo)
    {
        ...
        if ($selectTipo == 0) {
            ...
            $this->cursoos = DB::connection('mysql_segunda')->select('SELECT
                ...
                LEFT JOIN (
                    SELECT docente_curso.iddocente_curso, curss.carrera_malla, docente_curso.iddocente_curso as iddocentecurso, curss.año_de_inicio, docente_curso.idsemestre_academico, curss.idciclos, docente_curso.id_docente as dc_iddoce, docente_curso.idcursos as idcur, curss.idmalla_curricular, userprofile.nombre
                        FROM docente_curso
                        LEFT JOIN
    ...
    #[\Livewire\Attributes\On('confirmarGuardado')]
    public function agregar()
    {
        // dd($docenteId);
        $this->validate();
        // foreach ($this->cursoos as $curso) {
        //     if (empty($this->asignaciones[$curso->idcursos])) {
        //         $this->dispatch('mensaje-error', mensaje: "Falta asignar docente para el curso: {$curso->nombre_curso}");
        //         return;
        //     }
        // }
        try {
            DB::connection('mysql_segunda')->beginTransaction();
            foreach ($this->asignaciones as $cursoId => $docenteId) {
                if (!empty($docenteId)) {
                    $existe = DB::connection('mysql_segunda')->table('docente_curso')
                        ->where('idcursos', $cursoId)
                        ->where('idsemestre_academico', $this->selecdSemesAca)
                        ->where('tipodocente_curso', $this->selectTipo)
                        ->exists();

                    if (!$existe) {
                        $inser = DB::connection('mysql_segunda')->table('docente_curso')->insert([
                            'idcursos' => $cursoId,
                            'id_docente' => $docenteId,
                            'año_dictado' => now(),
                            'idsemestre_academico' => $this->selecdSemesAca,
                            'tipodocente_curso' => $this->selectTipo,
                        ]);
                    } else {
                        $updateDoce = DB::connection('mysql_segunda')->table('docente_curso')
                            ->where('idcursos', $cursoId)
            ...
            $this->traerCursos($this->selectTipo);

            $this->dispatch('cursos-actualizados');
            $this->dispatch('mensaje-exito', mensaje: 'Asignaciones guardadas correctamente');
        } catch (\Throwable $th) {
            DB::connection('mysql_segunda')->rollBack();
            $this->dispatch('cursos-actualizados');
            $this->dispatch('mensaje-exito', mensaje: 'Error -------------');
        }
    }

    public function confirmarGuardado()
    {
        $this->dispatch('mostrar-confirmacion');
    }

    #[\Livewire\Attributes\On('quitarDocente')]
    public function quitarDocente($iddocentecurso)
    {
        DB::connection('mysql_segunda')->table('docente_curso')
            ->where('iddocente_curso', $iddocentecurso)
            ->where('idsemestre_academico', $this->semesAca)
            ->update(['id_docente' => 0]);

        unset($this->asignaciones[$iddocentecurso]);

        $this->traerCursos($this->selectTipo);

        $this->dispatch('cursos-actualizados');
        $this->dispatch('docente-quitado');
    }


    public function render()
    {
        return view('livewire.Admin.asignar-docente');
    }
}
