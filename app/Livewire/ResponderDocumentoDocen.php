...
class ResponderDocumentoDocen extends Component
{
    use WithFileUploads;
    ...
    public function calcularCorrelativo($value = null)
    {
        ...
        if ($valor) {
            $ultimoExpediente = DB::connection('mysql_documentario')->table('documentos')
                ->where('emisor', session('dependencia_id'))
    ...
    public function submit()
    {
        ...
        try {
            ...
            //tomar el idmovimiento del documento
            $idmovimiento = DB::connection('mysql_documentario')->table('movimiento')
                ->where('iddocumentos', $this->iddocument)
                ->where('id_user_receptor', Auth::id())
                ->value('idmovimiento');
    ...
    {
        return view('livewire.responder-documento-docen');
    }
}
